...
## 2026-07-27–28 extended observation, repairs, and protected reset

The observation beginning at `2026-07-24T14:52:27.804Z` ran for more than 86 hours. Immediately before the final reset, the authoritative Rotator volume held 97 events, zero proposals, and the unchanged 25-rule ignore list. Replaying the complete window through Rotator `374292f` produces 53 auth/config records, 28 transient/external records, 13 code records, three expected/operator records, and zero unknowns. No provider draft or automatic patch was applied, and no ignore rule was added.

Most code records were paired echoes from image/provider defects already repaired by the July 27 StreamWeaver deployments. The remaining verified code defects were repaired narrowly: StreamWeaver `cd3ce12` falls back from the obsolete Crew URL to the shared Discord guild, keeps MountainView events bound to the StreamWeaver SPMT app ID, and emits bounded one-line Twitch/SPMT failures; Chat Tag `7289488` validates and normalizes pack art before ImageResponse rendering; Chat Tag `3cf9f7b` fixes the independently exposed Satori layout failure and adds an executable PNG-render regression test. Rotator `683e43c` and `374292f` learned all observed auth, provider, transport, code, fallback, and operator families deterministically. Its full 131-test suite, typecheck, and build passed.

The existing Chat Tag app-bound SPMT key received only `xp:write` and verified live as `appId=chat-tag` with `events:write`, `identity:write`, and `xp:write`; no secret was rotated and no app restart was required. Fly snapshot `vs_mJOXmMev9pJmh94gDP5VN` completed before the database mutation. An attempted online SQLite backup did not finish within the bounded SSH window; its orphaned shell and Node processes were identified by exact command line, terminated without touching the application server, and the incomplete files were removed. The snapshot is backup evidence, not isolated restore proof.

All affected Actions completed successfully. The final `npm run smoke:suite` at `2026-07-28T05:00:57.896Z` returned `200` for all ten app/worker health and feature routes and proved exact local `HEAD` = `origin/main` = deployed Fly SHA. The repaired Chat Tag pack-preview route separately returned `200 image/png` with a 294,082-byte response. The protected reset archived all 97 records and zero proposals at `/data/error-archives/2026-07-28T05-01-30-832Z` and wrote the new baseline at `2026-07-28T05:01:30.867Z`; events and proposals remained at zero through the delayed monitor check at `2026-07-28T05:02:56Z`, and the ignore list remains 25.

Operator-owned auth/config remains visible rather than ignored: one StreamWeaver tenant still lacks a broadcaster grant, other tenants need the required shared-chat or `clips:edit` Twitch scopes, SeaArt rejected an account token, several Discord channel/webhook mappings are stale or unavailable, a DSH Twitch mapping is banned, and the HMO DJ worker needs a usable YouTube cookie/provider path for challenged media. These are not safe coding-model fixes. The formal Gate 0 blocker remains the isolated restore, measured RPO/RTO, operator, and rollback drill for each authoritative store.

## 2026-07-30–31 classified window and protected reset

The authoritative Rotator volume held 60 events, zero proposals, and the unchanged 25-rule ignore list immediately before this reset. The final replay through Rotator `27dfa37` classifies all 60 records as 40 auth/config, 12 transient/external, and eight code-owned historical records, with zero unknowns and zero autofix-eligible records. The eight code records are six halves of the Chat Tag crown-module startup stack, one tenantless Quackverse overlay request, and one StreamWeaver 10 MB upload-envelope event. Current Chat Tag images copy and import-check the crown helper at build time, Chat Tag `1d6d54a` skips only tenant-scoped overlay delivery when no authoritative tenant exists, and StreamWeaver already runs the bounded 64 MB request envelope with a smaller route limit.

StreamWeaver `a19a198` pauses proactive refresh after a deterministic invalid-grant response for an unchanged token revision and resumes automatically when OAuth writes a new revision. The owner’s concurrent Discord-reply work merged immediately above it as final main/deployed `190f284`; the isolated OAuth branch passed its focused test, typecheck, lint, and 222-route production build before merge. Rotator `327bce1` and `27dfa37` learned the observed OAuth, packaging, tenantless-overlay, Kick/Pusher 1006, Fly PM07, split-stack, and already-repaired upload families. Its final suite passed 132 tests, typecheck, build, Actions, and Fly deployment. No ignore rule was added.

The remaining 40 auth/config records require provider or channel ownership rather than a generated patch: 25 invalid-refresh records for Twitch tenant `432778419`, four missing-broadcaster-grant records, four DSH banned-channel records, three unavailable Discord webhook records, two stale Discord mirror-channel records, and two Twitch login failures. The 12 transient records are bounded Twitch no-response/reconnect, Fly proxy reset, recovered SpaceMountain shoutout-forward timeout, and one duplicate machine-start lifecycle record.

All affected Actions succeeded. The final clean-workspace `npm run smoke:suite` at `2026-07-31T05:52:41.171Z` returned `200` for every health and feature route and proved exact clean local main = `origin/main` = deployed Fly SHA for all ten app/worker entries. The protected reset archived 60 events and zero proposals at `/data/error-archives/2026-07-31T05-50-42-827Z`, wrote `startedAt=2026-07-31T05:50:42.848Z`, and independently verified zero current events and zero current proposals. This begins a new post-fix 24-hour validation window; unresolved auth/config conditions must recur once and remain visible, while unchanged invalid OAuth grants should no longer create maintenance-loop spam.

The delayed monitor check at `2026-07-31T05:53:33Z` contained one event and zero proposals: the known DSH `msg_banned` authorization condition recurred and classified as `discord-stream-hub-new:twitch-channel-banned`, with no autofix. The current live-channel set minus DSH's joined-channel set narrows the affected candidate channels to `aryaeragonskyblustar`, `razorbladesplays236`, or `robdparry`; current TMI logging does not identify which rejection belongs to which channel. The channel owner must unban the configured Twitch bot or the stale mapping must be removed. Do not clear or ignore this record merely to display zero.
