...
2026-07-29 historical-balance correction: the July 24 DSH canonical dual-write began without migrating the existing Firestore leaderboard, and a July 28 guard then excluded Twitch chat activity from canonical awards. The DSH balance therefore remained the historical source while shared displays showed only post-cutover SPMT rows. The corrective implementation adds a one-time, app-bound migration that records the observed DSH balance and adds only the missing DSH contribution; it never lowers an existing ledger and cannot repeat for the same user/app. DSH supplies immutable Twitch IDs on forwarded events and resumes idempotent Twitch-message awards. Keep this checkbox open until a deployed signed-in read proves the historical balance and a later Twitch message increments both stores once.
