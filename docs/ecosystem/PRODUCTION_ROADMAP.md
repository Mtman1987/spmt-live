...
### 2026-07-30–31 Gate 0 replay, narrow repairs, and new validation window

- The authoritative pre-reset state contained 60 events, zero proposals, and the unchanged 25 narrow ignore rules. Rotator `27dfa37` replays the complete set as 40 auth/config, 12 transient/external, and eight already-repaired code records, with zero unknowns and zero autofix-eligible records. No ignore rule was added.
- Chat Tag `1d6d54a` no longer calls a tenant-scoped pack overlay from tenantless browser/Discord surfaces; the pack open still succeeds. Its latest bot image also contains the previously omitted crown helper and verifies that import during the Docker build. StreamWeaver `a19a198` pauses repeated proactive refreshes for an unchanged invalid Twitch grant and automatically retries after reauthorization changes the stored revision. The owner’s concurrent Discord reply cleanup merged above it as final StreamWeaver main `190f284`.
- The remaining auth/config conditions are intentionally visible and operator-owned: Twitch tenant `432778419` must reauthorize, one tenant lacks a broadcaster grant, one DSH Twitch mapping remains banned, and several Discord webhook/channel mappings are unavailable or stale. The app cannot mint provider grants or override channel permissions, and Rotator must not invent those fixes.
- StreamWeaver passed its focused OAuth test, typecheck, lint, and 222-route production build; Chat Tag passed typecheck and its 61-route production build; Rotator passed 132 tests, typecheck, and build. All affected Actions and Fly deployments succeeded.
- The final clean-workspace `npm run smoke:suite` at `2026-07-31T05:52:41.171Z` proved exact clean local main = `origin/main` = deployed Fly SHA plus successful health and feature routes for all ten app/worker targets. The protected reset archived 60 events and zero proposals at `/data/error-archives/2026-07-31T05-50-42-827Z`, established `startedAt=2026-07-31T05:50:42.848Z`, and verified zero current events and zero proposals. Keep the automation active through the new 24-hour post-fix validation window before certifying this release slice stable.
- The delayed monitor check at `2026-07-31T05:53:33Z` correctly re-opened one operator-owned DSH `msg_banned` event and created no proposal. Current live-versus-joined channel evidence narrows the candidate channels to `aryaeragonskyblustar`, `razorbladesplays236`, or `robdparry`, but current TMI output does not attribute the rejection exactly. Unban the configured Twitch bot or remove the stale mapping; do not clear or ignore this unresolved authorization record to manufacture a zero.
