...
| Image release at capture | Fly release 54 |
...
- Online logical backup script: added at `scripts/operations/backup-and-verify.mjs`.
- Isolated logical restore fixture: to be executed after the script is deployed; it verifies integrity and table counts without printing row data.
...
1. Deploy and verify the new liveness/readiness/build contract.
2. Run the logical backup and isolated restore rehearsal from the deployed image.
3. Restore a Fly snapshot to an isolated temporary volume/machine and measure RTO.
4. Establish retained logs and capture 24–48 hours of grouped errors.
5. Configure owner recovery with an operationally stored secret and exercise the locked-out-user flow.
6. Rotate all five OAuth client secrets with their consumer apps and remove code/database fallbacks.
7. Move public identifiers, origins, URLs, and operator flags to validated volume-backed public runtime JSON.
8. Add rate limits and audit records to recovery, login, token, webhook, and platform-trigger routes.
