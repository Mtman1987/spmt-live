(() => {
  ...
  function addCommunityLoungePreset() {
    const url = 'https://spmt.live/tenant/spacemountainlive/public';
    try {
      setStatus?.('SpaceMountainLive uses its own zero-config system Public scene.', 'ok');
    } catch {}
  ...
  function catalogMarkup() {
    ...
    return `<section class="spmt-app-overlay-catalog" data-app-overlay-catalog>
      ...
      <div class="spmt-app-overlay-preset"><button type="button" class="button primary" data-community-lounge-preset>Open SpaceMountainLive Lounge</button><span>System-owned zero-config live composition. No tenant, Twitch ID, room ID, or Nebula URL entry required.</span></div>
      ${body}
    </section>`;
  }
