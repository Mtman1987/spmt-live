(() => {
  ...
  function installStyle() {
    ...
    style.textContent = `
      ...
      .spmt-welcome-art{display:grid;place-items:center;min-height:150px}
      .spmt-welcome-art img{width:min(100%,270px);max-height:170px;object-fit:contain;filter:drop-shadow(0 0 24px rgba(var(--spmt-accent-rgb,249,115,22),.3))}
      .spmt-auth-hero{margin:6px 0 18px}
      #view-dashboard>.spmt-welcome-hero{grid-column:1/-1}
      .spmt-nav-glyph{display:inline-grid;place-items:center;min-width:22px;font-size:16px;line-height:1;color:var(--spmt-accent,#f97316);font-weight:900}
      .nav button{display:flex;align-items:center;gap:9px}
      #spmt-sidebar-collapse-toggle{width:100%;display:flex;align-items:center;justify-content:center;gap:7px;margin:-8px 0 10px;padding:7px 8px;border:1px solid var(--spmt-line);border-radius:999px;background:rgba(0,0,0,.2);color:var(--muted);font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:.08em}
      #spmt-sidebar-collapse-toggle:hover{color:#fff;border-color:var(--spmt-line-strong)}
      body.spmt-sidebar-collapsed:not(.spmt-sidebar-hidden) .sidebar .brand{justify-content:center}
      body.spmt-sidebar-collapsed:not(.spmt-sidebar-hidden) .sidebar .brand>div:not(.mark){display:none}
      body.spmt-sidebar-collapsed:not(.spmt-sidebar-hidden) .spmt-nav-label{display:none}
      body.spmt-sidebar-collapsed:not(.spmt-sidebar-hidden) .nav button{justify-content:center;padding-inline:8px}
      body.spmt-sidebar-collapsed:not(.spmt-sidebar-hidden) .sidebar .account .small,
      body.spmt-sidebar-collapsed:not(.spmt-sidebar-hidden) .sidebar .account strong,
      body.spmt-sidebar-collapsed:not(.spmt-sidebar-hidden) .sidebar .account .row{display:none}
      body.spmt-sidebar-collapsed:not(.spmt-sidebar-hidden) .sidebar .account{min-height:42px;padding:8px;background:rgba(var(--spmt-accent-rgb,249,115,22),.08)!important}
      body.spmt-sidebar-collapsed:not(.spmt-sidebar-hidden) #spmt-sidebar-collapse-toggle .spmt-collapse-label{display:none}
      .spmt-app-art{height:82px;margin:-2px 0 13px;display:grid;place-items:center;border:1px solid var(--spmt-line);border-radius:min(var(--spmt-radius,18px),16px);background:rgba(0,0,0,.23);overflow:hidden}
      .spmt-app-art img{width:72px;height:72px;object-fit:contain;filter:drop-shadow(0 0 12px rgba(var(--spmt-accent-rgb,249,115,22),.18))}
      .spmt-app-art.wide img{width:138px;height:66px}
      ...
      .spmt-tray-bar{display:flex;align-items:center;gap:8px;min-height:58px;padding:8px 10px}
      .spmt-tray-workspace,.spmt-tray-slot{border:1px solid rgba(255,255,255,.09);border-radius:12px;background:rgba(255,255,255,.035);color:#fff}
      .spmt-tray-workspace{display:flex;align-items:center;gap:8px;flex:0 0 auto;padding:10px 12px;font-size:12px;font-weight:900}
      .spmt-tray-workspace span:first-child{color:var(--spmt-accent,#f97316);font-size:17px}
      .spmt-tray-slots{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:6px;min-width:0;flex:1}
      .spmt-tray-slot{min-width:0;padding:8px 10px;text-align:left}
      .spmt-tray-slot strong{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:10px}
      .spmt-tray-slot small{display:block;margin-top:3px;color:#71788a;font-size:8px;font-weight:800;text-transform:uppercase;letter-spacing:.08em}
      .spmt-tray-slot.active,.spmt-tray-workspace.active{border-color:var(--spmt-line-strong);background:rgba(var(--spmt-accent-rgb,249,115,22),.13)}
      .spmt-tray-panel{border-top:1px solid rgba(255,255,255,.08);background:rgba(0,0,0,.3)}
      .spmt-tray-panel-head{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:9px 12px}
      .spmt-tray-panel-head strong{display:block;font-size:12px}.spmt-tray-panel-head small{display:block;margin-top:2px;color:#747b8d;font-size:9px;text-transform:uppercase;font-weight:800}
      .spmt-tray-panel-actions{display:flex;align-items:center;gap:7px}.spmt-tray-panel-actions a,.spmt-tray-panel-actions button{border:1px solid rgba(255,255,255,.1);border-radius:9px;background:rgba(255,255,255,.035);color:#c7ceda;padding:6px 9px;text-decoration:none;font-size:9px;font-weight:800}
      .spmt-tray-frame-wrap{position:relative;height:min(48vh,430px);border-top:1px solid rgba(255,255,255,.08);background:#030408}
      .spmt-tray-frame-wrap iframe{position:absolute;inset:0;width:100%;height:100%;border:0;background:#030408}
      .spmt-tray-empty{height:100%;display:grid;place-items:center;padding:20px;color:#737b8e;text-align:center;font-size:12px}
      body.spmt-runtime-mounted .main{padding-bottom:96px!important}
      body.spmt-runtime-mounted .commlink-frame{height:calc(100vh - 190px)!important;min-height:520px}
      #spmt-overlay-runtime{position:fixed;inset:0;z-index:48;pointer-events:none;overflow:hidden}
      .spmt-overlay-widget{position:absolute;overflow:hidden;border-radius:12px;transition:opacity .15s ease;transform-origin:center}
      .spmt-overlay-widget iframe{width:100%;height:100%;border:0;background:transparent}
  ...
  function makeHero(kind) {
    ...
    hero.innerHTML = `
      <div class="spmt-welcome-copy">
        ...
        <h2>${kind === 'home' ? `Welcome back, ${escapeHtml(name)}.` : 'Welcome to SpaceMountain.'}</h2>
        <p>${kind === 'home'
          ...
          : 'Sign in once with SPMT to carry the same identity, workspace, apps, messages, Worktray, and overlay setup across the SpaceMountain ecosystem.'}</p>
  ...
  function installHeroes() {
    ...
    if (dashboard && !dashboard.querySelector('[data-spmt-hero="home"]')) {
      ...
      if (heading) heading.textContent = `Welcome back, ${displayName()}.`;
    }
  }
    ...
    try {
      ...
      const response = await fetch('/api/workspace-profile', {
        ...
        headers: {
          ...
          'If-Match': etag || `"workspace-${profile.revision}"`,
        },
  ...
  function currentTrayTarget() {
    if (state.trayTarget.kind === 'workspace') {
      return {
        ...
        subtitle: 'SPMT canonical Worktray',
        url: '/embed/worktray?mode=dock&app=spmt',
      };
    }
  ...
  function renderWorktray() {
    ...
    tray.innerHTML = `
      ...
      ${state.trayOpen ? `<div class="spmt-tray-panel">
        <div class="spmt-tray-panel-head">
          <div><strong>${escapeHtml(target.title)}</strong><small>${escapeHtml(target.subtitle)}</small></div>
          <div class="spmt-tray-panel-actions">${target.url ? `<a href="${escapeHtml(target.url)}" target="_blank" rel="noreferrer">Pop out</a>` : ''}<button type="button" data-spmt-tray-close>Collapse</button></div>
        </div>
        <div class="spmt-tray-frame-wrap">${target.url ? `<iframe src="${escapeHtml(target.url)}" title="${escapeHtml(target.title)}" allow="autoplay; microphone; camera; fullscreen; clipboard-write"></iframe>` : '<div class="spmt-tray-empty">No URL is assigned to this workspace slot yet.</div>'}</div>
      </div>` : ''}`;
    document.body.append(tray);
    document.body.classList.add('spmt-runtime-mounted');
    tray.querySelector('[data-spmt-tray-workspace]')?.addEventListener('click', () => {
      state.trayTarget = { kind: 'workspace', slotId: null };
      state.trayOpen = true;
      renderWorktray();
    });
    tray.querySelectorAll('[data-spmt-tray-slot]').forEach((button) => button.addEventListener('click', () => {
      state.trayTarget = { kind: 'slot', slotId: Number(button.dataset.spmtTraySlot) };
      state.trayOpen = true;
      renderWorktray();
    }));
    tray.querySelector('[data-spmt-tray-close]')?.addEventListener('click', () => {
  ...
  function renderOverlay() {
    ...
    visible
      .map((widget, index) => ({ widget, layer: Number.isFinite(Number(widget.zIndex)) ? Number(widget.zIndex) : index + 1 }))
  ...
  async function loadRuntimeState() {
    ...
    try {
      ...
      if (overlayResponse.ok) {
        ...
        state.overlay = { enabled: true, widgets: [], workflows: [] };
      }
