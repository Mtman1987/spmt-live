...
test('tenantId is only a hint: snapshot includes ecosystem records from other tenants and systems', () => {
  ...
  try {
    db.prepare(`INSERT INTO messages
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run('m-a', 'room-a', '', 'tenant A chat', 'twitch', 'message', JSON.stringify({ provider: 'twitch', tenantId: 'tenant-a' }), '[]', '2026-08-17T20:52:00.000Z', 'tenant-a-user', 'tenant-a-user');
    db.prepare(`INSERT INTO messages
      (id, conversation_id, subject, body, channel, message_type, metadata, attachments, created_at, from_id, to_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run('m-b', 'room-b', '', 'tenant B chat', 'discord', 'message', JSON.stringify({ provider: 'discord', tenantId: 'tenant-b' }), '[]', '2026-08-17T20:53:00.000Z', 'tenant-b-user', 'tenant-b-user');
    ...
    const snapshot = buildSnapshot(db, {
      since: '2026-08-17T20:50:00.000Z',
      until: '2026-08-17T21:00:00.000Z',
      tenantId: 'tenant-a',
      limit: '100',
...
test('diagnostic payload redacts common credentials recursively', () => {
  const redacted = redactValue({
    ...
    nested: {
      ...
      text: 'request failed?access_token=abc123&mode=debug',
    },
  });
