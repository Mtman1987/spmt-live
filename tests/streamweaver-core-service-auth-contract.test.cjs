...
test('StreamWeaver receives only its approved core service scopes', () => {
  assert.match(scopePatch, /ensureClientScopes\('streamweaver', \['entitlements:read', 'events:write', 'account-recovery:write', 'chat-tag:blacklist:read', 'jobs:read', 'jobs:write'\]\)/);
