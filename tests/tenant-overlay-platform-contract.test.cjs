...
test('Overlay Bay v3 exposes tenant outputs and standardized source controls', () => {
  ...
  for (const marker of [
    ...
    'https://hearmeout-main.fly.dev/spotlight-media',
    'obv3-interacting',
...
test('mtman1987 debug Lounge preset contains the complete populated scene', () => {
  ...
  assert.equal(
    ...
    'https://hearmeout-main.fly.dev/spotlight-media/player?v=live-spotlight-3',
  );
...
test('mtman1987 24/7 migration preserves sources and assigns the broadcast panels', () => {
  ...
  assert.equal(layout.widgets.find((widget) => widget.id === 'sw-shoutout')?.visible, true);
  ...
  assert.equal(media?.url, 'https://hearmeout-main.fly.dev/lounge-media/player?v=live-lounge-1');
...
test('saved hidden Lounge layers stay hidden and tenant renderer respects visibility in every mode', () => {
  ...
  assert.match(bootstrapSource, /interactiveTestIndex[\s\S]+splice\(interactiveTestIndex, 1\)/);
